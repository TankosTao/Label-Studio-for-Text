...
class Evolution(nn.Module):
    ...
    def evolve_poly_test(self, snake, cnn_feature,i_it_poly, c_it_poly):
        ...
        init_input = torch.cat([init_feature, c_it_poly.permute(0,2,1)], dim=1)
        init_input = init_input.type(torch.float32).to(cnn_feature.device)
        # get offsets
...
class Decode(torch.nn.Module):
    # c_in should be 64
    def __init__(self, c_in=256, num_point=128,ro=1.0, evolve_stride=4., down_sample=4., thresh=0.3,box_thresh=0.5,epsilon=0.002,min_size=4,is_output_polygon=True,ignore=False):
